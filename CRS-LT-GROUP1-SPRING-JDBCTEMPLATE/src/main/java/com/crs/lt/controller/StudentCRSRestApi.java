...
public class StudentCRSRestApi {
	...
	private static Logger logger = Logger.getLogger(StudentCRSRestApi.class);

	@RequestMapping(produces = MediaType.APPLICATION_JSON, method = RequestMethod.GET, value = "/courses")
	@ResponseBody
	public ResponseEntity<?> viewCourses(@RequestParam(value ="studentId") String studentId) throws SQLException {
...
//	    }
	...
	public ResponseEntity<?> addCourse(@RequestParam(value = "courseCode") String courseCode,
		...
		if (registrationInterface.addCourse(courseCode, studentId, availCourseList)) {
			...
			return ResponseEntity.status(201).body("Course not added : " + courseCode);
